...
export const BatchGenerator: React.FC<BatchGeneratorProps> = ({
  ...
  const [selectedTemplateIds, setSelectedTemplateIds] = useState<string[]>(
    templates
      .filter((t) => t.id === "SKU_001" || t.id === "MAIN_001")
      .map((t) => t.id),
  );

  // Render variables defaults
  const [exportFormat, setExportFormat] = useState<"JPG" | "PNG" | "WebP">(
    "JPG",
  );
  ...
  const [currentSynthesizingTask, setCurrentSynthesizingTask] =
  ...
  const launchSynthesisEngine = async () => {
    ...
    for (const prodId of selectedProductIds) {
      ...
      for (const tempId of selectedTemplateIds) {
        ...
        // 1. PNG缺失 check:
        const hasPng = prod.assets.some(
          (a) => a.assetType === "transparent_png" && a.status === "ready",
        );
        if (!hasPng) {
        ...
        // 2. 产品越界 check:
        const slotsTooBig = temp.slots.some(
          (s) => s.maxWidth > 80 || s.maxHeight > 60,
        );
        const isBigItem =
          prod.size &&
          (prod.size.includes("240mm") || prod.size.includes("260mm"));
        ...
        // 3. 文案越界 check:
        const extraLongText =
          (prod.productName || "").length + (prod.seriesName || "").length > 8;
        if (extraLongText && temp.textFields.some((tf) => tf.isDynamic)) {
          qualityIssues.push(
            "文案越界: 所属系列与排套名称字轨数超出文字标定安全区",
          );
        }
        ...
        try {
          ...
          try {
            ...
            console.warn(
              "Base fusion pre-render failed, fallback to main standard URL:",
              err,
            );
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-slate-100 mt-1 text-[10px]">
                    ...
                    <span
                      className={`font-mono text-[9px] font-bold ${isEligible ? "text-emerald-600" : "text-amber-600"}`}
                    >
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center pb-2 border-b border-slate-100">
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-1">
                勾选需要自动合成何种模板。所勾选的模板类型将被组合套入选定产品中合集生成。
              </p>
            </div>
            <div className="text-xs text-slate-650">
              已勾选模板:{" "}
      ...
      {step === 3 && (
        <div className="space-y-4">
          ...
          <div className="bg-slate-50/65 p-4 rounded-xl border border-slate-150 space-y-3 pb-5 text-xs">
            ...
            {[
              {
                slot: "[productName] (主副标题)",
                source: "product.productName",
                render: "文字自适应不拉伸",
              },
              {
                slot: "[productCode] (款式编号)",
                source: "product.productCode",
                render: "大写单股等宽不切",
              },
              {
                slot: "[size] (材质细节)",
              ...
              {
                slot: "[materialCover] (材料纸况)",
                source: "product.materialCover",
                render: "缩排12px",
              },
              {
                slot: "front_cover (封面正面槽)",
                source: "assets.transparent_png",
                render: "等比裁剪底边锁定",
              },
              {
                slot: "inner_page (内页槽位)",
                source: "assets.inner_page",
                render: "多重层叠层35%露白",
              },
            ].map((mapItem, index) => (
              <div
                key={index}
                className="grid grid-cols-3 py-2 border-b border-dashed border-slate-150 last:border-0"
              >
                ...
                <span className="text-slate-500 text-right font-semibold">
                  ✓ {mapItem.render}
                </span>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs bg-slate-50/50 p-4 rounded-xl border border-slate-200">
            <div>
              <span className="block text-slate-450 font-bold mb-1.5 uppercase text-[10px]">
                输出文件格式
              </span>
              <select
                value={exportFormat}
          ...
          <div className="p-4 rounded-xl border border-blue-200 bg-blue-50/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs leading-normal">
            <div className="space-y-0.5">
              <span className="font-bold flex items-center text-slate-800">
                🛠️ 台历资产验证模式：
                <span
                  className={`ml-1.5 font-black uppercase font-mono px-2 py-0.5 rounded text-[10px] ${demoMode ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}
                >
                  {demoMode
                    ? "DEMO 模式 (允许矢量占位图)"
                    : "生产模式 (必须使用真实 PNG)"}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-50/80 border border-slate-200/85 p-4 rounded-xl flex flex-col justify-between text-xs text-slate-700">
              <div className="space-y-4">
                <h4 className="font-bold text-slate-800 border-b border-slate-150 pb-2">
                  智能排版对齐逻辑规范
                </h4>
                <div className="space-y-2.5">
                  <div className="flex items-start space-x-2 text-[11px] leading-normal text-slate-600">
                    ...
                    <span>
                      产品宽高比约束：已锁定 100% 保持实物几何真实现象。
                    </span>
                  </div>
                  <div className="flex items-start space-x-2 text-[11px] leading-normal text-slate-600">
                    <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                    <span>
                      铁骨/线圈保护：凡在槽内拉伸裁切线圈，系统自动触发退回风险标。
                    </span>
                  </div>
                  <div className="flex items-start space-x-2 text-[11px] leading-normal text-slate-600">
                    <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                    <span>
                      文案溢出质检：由于名字长度引发越界的文字图层将被阻断打回。
                    </span>
                  </div>
                </div>
              </div>
