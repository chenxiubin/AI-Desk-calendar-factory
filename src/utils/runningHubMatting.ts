...
export async function runRunningHubMatting({
  ...
  const imageNodeId =
    workflowConfig.inputImageNodeId || workflowConfig.baseImageNodeId;
  if (!imageNodeId) {
  ...
  if (imageUrlOrBase64) {
    ...
    const uploadResult = await uploadImageToRunningHub(blob, workflowConfig);
  ...
  const result = await createRunningHubTask({
    ...
    apiBaseUrl: workflowConfig.apiBaseUrl,
